...
import type { FluxColumn, TableTransform } from '../interfaces'

type NormalizeAsArrayInput<T> = null | T | T[]

function normalizeAsArray(input: NormalizeAsArrayInput<FluxColumn>): FluxColumn[] {
  if (input == null) {
    return []
...
export default function traverseColumn(
  ...
  ) => NormalizeAsArrayInput<FluxColumn>,
): TableTransform {
  return ({ columns, dataSource }) => {
    return { dataSource, columns: dfs(columns, 0).result }

    function dfs(columns: FluxColumn[], parentStartColIndex: number): { flatColCount: number; result: FluxColumn[] } {
      let flatColCount = 0
