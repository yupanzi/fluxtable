...
import type { FluxColumn, SpanRect } from '../interfaces'
import { internals } from '../internals'
import collectNodes from './collectNodes'
import getTreeDepth from './getTreeDepth'
import isLeafNode from './isLeafNode'

function safeGetSpanRect(column: FluxColumn, record: any, rowIndex: number, colIndex: number): SpanRect {
  let colSpan = 1
  let rowSpan = 1
...
export default function exportTableAsExcel(
  ...
  function addTopHeaders(origin: CellAddress) {
    ...
    function dfs(cols: FluxColumn[], startDx: number, startDy: number) {
      ...
      for (const col of cols) {
        ...
        if (isLeafNode(col) || col.children == null) {
          offsetX += 1
