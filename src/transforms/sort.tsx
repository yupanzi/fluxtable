...
import type { FluxColumn, SortItem, SortOrder, TableTransform } from '../interfaces'
...
function hasAnySortableColumns(cols: FluxColumn[]): boolean {
  return cols.some(
  ...
  return ({ dataSource, columns }) => {
    if (process.env.NODE_ENV !== 'production') {
      if (!hasAnySortableColumns(columns)) {
        console.warn(
          '[fluxtable] commonTransform.sort 缺少可排序的列，请通过 column.features.sortable 来指定哪些列可排序',
          columns,
        )
      }
    }
    ...
    function processColumns(columns: FluxColumn[]) {
      return columns.map(dfs)

      function dfs(col: FluxColumn): FluxColumn {
        const result = { ...col }

        const code = col.code
        const sortable = code != null && (Boolean(col.features?.sortable) || sortMap.has(code))
        const active = sortable && sortMap.has(code)
        ...
        if (sortable) {
          ...
          if (active) {
            const sortEntry = sortMap.get(code)
            if (sortEntry != null) {
              sortOrder = sortEntry.order
              sortIndex = sortEntry.index
            }
