...
import type { FluxColumn, FluxColumnStaticPart, CellProps } from '../../interfaces'
...
export interface MultiSelectFeatureOptions {
  ...
  checkboxColumn?: Partial<FluxColumnStaticPart>
...
export function multiSelect(opts: MultiSelectFeatureOptions = {}) {
  return function multiSelectStep(pipeline: TablePipeline) {
    ...
    const checkboxColumn: FluxColumn = {
      name: '是否选中',
        ...
        return (
          <Checkbox
            ...
            onChange={
              clickArea === 'checkbox'
                ? (arg1: any, arg2: any) => {
                    // 兼容两类 Checkbox 组件的 onChange 回调签名：
                    // 一类签名为 (checked, event)，原生事件在第二个参数；
                    // 另一类签名为 (event)，原生事件在第一个参数。
