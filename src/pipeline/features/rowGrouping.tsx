...
import type { CellProps, FluxColumn, SpanRect } from '../../interfaces'
...
function rowGroupingRowPropsGetter(row: any, _rowIndex: number): React.HTMLAttributes<HTMLTableRowElement> {
...
export function rowGrouping(opts: RowGroupingFeatureOptions = {}) {
  return (pipeline: TablePipeline) => {
    ...
    const onChangeOpenKeys: NonNullable<RowGroupingFeatureOptions['onChangeOpenKeys']> = (nextKeys, key, action) => {
    ...
    function processColumns(columns: FluxColumn[]) {
      ...
      const getCellProps = (value: any, row: any, rowIndex: number): CellProps => {
        const meta = getGroupingMeta(row)
      ...
      return [
        {
          ...
          getSpanRect(value: any, row: any, rowIndex: number): SpanRect | undefined {
            ...
            // 非分组头行不进行单元格合并，返回 undefined（消费端按 colSpan/rowSpan=1 处理）
            return undefined
          },
        },
        ...others,
