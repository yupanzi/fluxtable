...
import type { FluxColumn, TableTransform, Transform } from '../interfaces'
...
export class TablePipeline {
  ...
  /** 设置流水线的输入数据 */
  input(input: { dataSource: any[]; columns: FluxColumn[] }) {
  ...
  /** @deprecated
   *  应用一个 fluxtable Table transform */
  ...
  /** 转换 columns */
  mapColumns(mapper: Transform<FluxColumn[]>) {
    ...
    if (this._rowPropsGetters.length > 0) {
      result.getRowProps = (row, rowIndex) => {
        return this._rowPropsGetters.reduce<any>((res, get) => {
          // 注：mergeCellProps 形参类型为单元格属性，此处复用它来合并「行属性」，
          // 行/单元格属性结构兼容但 TS 类型不同，沿用既有 as any 做桥接（与 null 检查无关）。
