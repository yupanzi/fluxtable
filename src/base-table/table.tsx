...
import type { RenderInfo, ResolvedUseVirtual, VerticalRenderRange, VirtualEnum } from './interfaces'
import Loading, { type LoadingContentWrapperProps } from './loading'
import { type BaseTableCSSVariables, Classes, LOCK_SHADOW_PADDING, StyledFluxTableWrapper } from './styles'
import {
  getScrollbarSize,
  OVERSCAN_SIZE,
  shallowEqual,
...
function warnEmptyContentIsDeprecated() {
  if (!emptyContentDeprecatedWarned) {
    ...
    console.warn(
      '[fluxtable] BaseTable props.emptyContent 已经过时，请使用 props.components.EmptyContent 来自定义数据为空时的表格表现',
    )
  }
}
...
function warnFlowRootIsDeprecated() {
  if (!flowRootDeprecatedWarned) {
    ...
    console.warn('[fluxtable] BaseTable v2.4 版本之后已经不再需要指定 flowRoot')
  }
}

export type PrimaryKey = string | ((row: any) => string)

const defaultGetRowProps: NonNullable<BaseTableProps['getRowProps']> = () => ({})
...
export interface BaseTableProps {
  ...
  components?: {
    ...
    Cell?: React.ComponentType<{ row: any; rowIndex: number; colIndex: number; tdProps: unknown; column: FluxColumn }>
    /** 覆盖内部渲染 tbody 元素的组件 */
...
export class BaseTable extends React.Component<BaseTableProps, BaseTableState> {
  ...
  private rowHeightManager = makeRowHeightManager(this.props.dataSource.length, this.props.estimatedRowHeight ?? 48)

  private fluxTableWrapperRef = React.createRef<HTMLDivElement>()
  ...
  getDoms() {
    console.warn('[fluxtable] BaseTable.getDoms() 已经过时')
  ...
  private updateStickyScroll() {
    const { stickyScroll, fluxTable, stickyScrollItem } = this.domHelper

    const tableBodyHtmlTable = this.domHelper.getTableBodyHtmlTable()
    const innerTableWidth = tableBodyHtmlTable.offsetWidth
    const fluxTableWidth = fluxTable.offsetWidth

    const stickyScrollHeightProp = this.props.stickyScrollHeight ?? 'auto'
    ...
    if (fluxTableWidth >= innerTableWidth) {
  ...
  render() {
    ...
    const fluxTableWrapperClassName = cx(
      Classes.fluxTableWrapper,
      {
    ...
    const fluxTableWrapperProps = {
      className: fluxTableWrapperClassName,
      style,
      [STYLED_REF_PROP]: this.fluxTableWrapperRef,
    }

    return (
      <StyledFluxTableWrapper {...fluxTableWrapperProps}>
        <Loading
          visible={isLoading}
          LoadingIcon={components.LoadingIcon}
          LoadingContentWrapper={components.LoadingContentWrapper}
        >
          <div className={Classes.fluxTable}>
    ...
    this.rootSubscription.add(
      combineLatest([
        ...
        this.props$.pipe(
          op.startWith(null),
          op.pairwise(),
          op.filter(([prevProps, props]) => prevProps == null || (!prevProps.isLoading && props?.isLoading === true)),
        ),
      ]).subscribe(([clipRect]) => {
        ...
        // fixme 这里 loading 指示器的垂直定位在某些特殊情况下可能出现偏差
  ...
  private updateDOMHelper() {
    const fluxTableWrapper = this.fluxTableWrapperRef.current
    if (fluxTableWrapper == null) {
      throw new Error('[fluxtable] 表格根节点尚未挂载，无法初始化 TableDOMHelper')
    }
    this.domHelper = new TableDOMHelper(fluxTableWrapper)
  }
  ...
  private adjustNeedRenderLock() {
    ...
    if (hasLockColumn) {
      const sumOfColWidth = sum(flat.full.map((col) => col.width ?? 0))
      const nextNeedRenderLock = sumOfColWidth > this.domHelper.fluxTable.clientWidth
