...
function resolveVirtualEnabled(virtualEnum: VirtualEnum | undefined, defaultValue: boolean) {
...
function warnLockColumnNeedSpecifiedWidth(column: FluxColumn) {
  if (!lockColumnNeedSpecifiedWidthWarned) {
    lockColumnNeedSpecifiedWidthWarned = true
    console.warn('[fluxtable] lock=true 的列需要指定宽度', column)
  }
}

let columnHiddenDeprecatedWarned = false
function warnColumnHiddenDeprecated(column: FluxColumn) {
  if (!columnHiddenDeprecatedWarned) {
    columnHiddenDeprecatedWarned = true
    console.warn('[fluxtable] column.hidden 已经过时，如果需要隐藏该列，请将其从 columns 数组中移除', column)
  }
}

/** 检查列配置 & 设置默认宽度 & 剔除隐藏的列 */
function processColumns(columns: FluxColumn[], defaultColumnWidth: number | undefined) {
  if (columns == null || !Array.isArray(columns)) {
    console.warn('[fluxtable] <BaseTable /> props.columns 需要传入一个数组', columns)
    columns = []
  }

  function dfs(columns: FluxColumn[]): FluxColumn[] {
    const result: FluxColumn[] = []
...
function getLeftNestedLockCount(columns: FluxColumn[]) {
  let nestedCount = 0
  ...
  function isLock(col: FluxColumn): boolean {
    if (isLeafNode(col)) {
      return col.lock ?? false
    } else {
      return (col.lock ?? false) || (col.children?.some(isLock) ?? false)
    }
  }
}
...
function getHorizontalRenderRange({
  ...
  const rightBlank = sum(flat.center.slice(flat.center.length - rightBlankCount).map((col) => col.width ?? 0))
  return {
    leftIndex: leftIndex,
    leftBlank,
...
// 计算本次渲染所需的全部派生数据（可见列、行、跨度等）
// 备注：header 部分的计算逻辑目前在别处，与此处存在一定重复，后续可考虑合并收敛
export function calculateRenderInfo(table: BaseTable): RenderInfo {
  ...
  const unfilteredVisibleColumnDescriptors: (VisibleColumnDescriptor | false)[] = [
    ...flat.left.map((col, i) => ({ type: 'normal', col, colIndex: i } as const)),
    leftBlank > 0 && ({ type: 'blank', blankSide: 'left', width: leftBlank } as const),
    ...flat.center
      .slice(leftIndex, rightIndex)
      .map((col, i) => ({ type: 'normal', col, colIndex: flat.left.length + leftIndex + i } as const)),
    rightBlank > 0 && ({ type: 'blank', blankSide: 'right', width: rightBlank } as const),
    ...flat.right.map(
      (col, i) => ({ type: 'normal', col, colIndex: flat.full.length - flat.right.length + i } as const),
    ),
  ]
  const visibleColumnDescriptors = unfilteredVisibleColumnDescriptors.filter(
    (descriptor): descriptor is VisibleColumnDescriptor => descriptor !== false,
  )
  ...
  for (let i = 0; i < leftFlatCount; i++) {
    ...
    stickyLeft += flat.full[i].width ?? 0
  }
  ...
  for (let i = 0; i < rightFlatCount; i++) {
    ...
    stickyRight += flat.full[fullFlatCount - 1 - i].width ?? 0
  }

  const leftLockTotalWidth = sum(flat.left.map((col) => col.width ?? 0))
  const rightLockTotalWidth = sum(flat.right.map((col) => col.width ?? 0))
