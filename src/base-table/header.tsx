...
function filterNestedCenter(centerNested: FluxColumn[], hoz: HorizontalRenderRange, leftFlatCount: number) {
  return dfs(centerNested, leftFlatCount).filtered

  function dfs(cols: FluxColumn[], startColIndex: number) {
    let leafCount = 0
...
function attachColIndex(inputNested: FluxColumn[], colIndexOffset: number) {
  return dfs(inputNested, colIndexOffset).result

  function dfs(input: FluxColumn[], startColIndex: number) {
